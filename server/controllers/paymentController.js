      ...
      if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
        ...
        throw new Error('Razorpay key_id or key_secret is missing. Check your .env file and environment setup.');
      }
      ...
      try {
        ...
        console.log('Initializing Razorpay with:', {
          ...
          key_secret: process.env.RAZORPAY_KEY_SECRET ? '****' + process.env.RAZORPAY_KEY_SECRET.slice(-4) : undefined
        });
      ...
      export const createPaymentIntent = async (req, res) => {
        try {
          ...
          if (!course.published || !course.approved) {
            console.error('Course is not available:', courseId, { published: course.published, approved: course.approved });
          ...
          try {
            ...
            if (existingPendingPayment) {
              console.log('Found existing pending payment, returning it:', existingPendingPayment);
              return res.json({
      ...
      export const verifyPayment = async (req, res) => {
        try {
          ...
          console.log('Signature debug:', {
            ...
            keySecret: process.env.RAZORPAY_KEY_SECRET ? '****' + process.env.RAZORPAY_KEY_SECRET.slice(-4) : undefined
          });
          ...
          if (razorpay_signature !== expectedSign) {
            ...
            return res.status(400).json({ message: 'Invalid payment signature', expectedSign, receivedSignature: razorpay_signature });
          }
          ...
          if (!payment) {
            ...
            return res.status(404).json({ message: 'Payment not found', orderId: razorpay_order_id });
          }
